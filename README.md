...
Кнопка `⧉` растягивает терминал на всё окно, а половины клавиатуры
...
Кнопка `⌨↓` вынимает клавиатуру из окна целиком: терминал занимает всё
...
Пока клавиатура убрана, `⧉` и `⌨−`/`⌨+` выключены: накладывать и
масштабировать нечего. Поворот в это время не возвращает клавиатуру, но
запоминается — по нажатию `⌨↑` (та же кнопка, стрелка перевернулась) появится
та раскладка, которая положена текущей форме окна.
...
ширину, а размер клавиш задан шириной окна. Кнопки `⧉` и `⌨−`/`⌨+`
...
Ярлык называется `org.terkb.Terminal.desktop`, иконка — `org.terkb.Terminal`,
и тем же именем представляется само окно (`GLib.set_prgname`). Это не
формальность: под Wayland оболочка связывает окно с ярлыком только по
`app_id`, а GTK берёт его из имени программы — то есть «python3», раз запуск
идёт через `python3 -m terkb`. Пока имена расходились, GNOME не находил ярлык:
в доке значок оставался пустым, а у окна была чужая иконка-шестерёнка. Под X11
за то же отвечает `WM_CLASS`, поэтому в ярлыке прописан и `StartupWMClass`.

Короткое имя `terkb` остаётся запасным: иконка ставится под обоими именами,
и приложение ищет сначала по `app_id`, потом по нему.
...
На кнопках панели значки, а не подписи: экран планшета узкий, а слова
«Поверх» и «Скрыть ⌨» занимали столько же места, сколько три кнопки со
значками. Что кнопка делает — написано в подсказке; на тачскрине она
вызывается долгим нажатием. Там же названы текущая схема и текущий шрифт:
кнопки-циклеры больше не подписаны своим значением, зато панель влезает
в одну строку целиком.

Значки — обычные символы Unicode, а не файлы иконок: тема иконок в системе
может быть какой угодно, а шрифт интерфейса есть всегда. У лупы и кисти
дописан текстовый селектор `U+FE0E` — без него они приезжают цветными эмодзи
и не перекрашиваются вместе с остальной панелью. В коде после самого знака его
не видно, поэтому в `toolbar()` про него сказано в комментарии.
...
* **`🖌`** — цветовая схема по кругу, см. ниже. Какая выбрана сейчас — в
  подсказке.
* **`Aa`** — моноширинный шрифт терминала по кругу; выбранный назван в
  подсказке. Неактивна, если в системе стоит только один.
...
* **Восемь программируемых клавиш `M1`…`M8`** — четыре в правой половине и
  ...
  команды, см. ниже.
* **`＋`** — новая вкладка. Вкладок до восьми, переключаются полосой под
  панелью, см. ниже.
...
* **`⧉`** — переключение в режим наложения и обратно.
* **`⌨↓`** — убрать клавиатуру совсем, терминал на всё окно. Стрелка на кнопке
  переворачивается (`⌨↑`), и повторное нажатие возвращает ту раскладку,
  которая положена по форме окна.
...
кнопкой `🖌` по кругу: на планшете выбирать пункт из списка пальцем неудобно.
Название выбранной схемы — в подсказке к кнопке; на глаз она и так видна, ведь
схема красит всё окно.
...
Кнопка `＋` открывает вкладку с ещё одним шеллом. Вкладок бывает до восьми:
дальше подписи на полосе ужимаются до многоточия, и попасть пальцем в нужную
становится лотереей.

Полоса вкладок появляется под панелью инструментов, когда вкладок больше
одной, и исчезает, когда остаётся одна: на десятидюймовом экране каждая
строка интерфейса на счету, а переключать нечего. Под панелью, а не над ней —
чтобы кнопки не прыгали при открытии и закрытии вкладки. Как и панель, полоса
сделана `FlowBox`: минимальная ширина `Box`а — сумма всех вкладок, и она
стала бы полом для ширины окна, а `FlowBox` переносит лишние на вторую строку.

На вкладке — её номер и хвост заголовка, который выставил шелл: из
`пользователь@машина: ~/каталог` полезен только каталог, начало у всех вкладок
одинаковое. Номер стоит всегда — соседние вкладки обычно открыты в одном
месте, и без номера их подписи не различить. Полный заголовок — в подсказке.
Открытая вкладка закрашена фоном терминала и подписана жирным: акцентным
цветом её красить нельзя, у половины схем выделение почти не отличается от
панели, и открытая вкладка терялась.

Крестик рядом с подписью закрывает вкладку; шелл при этом получает `SIGHUP`,
как при закрытии обычного окна терминала. Обычный же путь — выйти из шелла
(`exit` или `Ctrl+D`): вкладка закрывается сама. Когда закрывается последняя,
закрывается и окно — как было до вкладок.

С подключённой настоящей клавиатурой работают привычные сочетания:
`Ctrl+Shift+T` — новая вкладка, `Ctrl+Shift+W` — закрыть, `Ctrl+PgUp` и
`Ctrl+PgDn` — соседняя по кругу. Экранной клавиатуре они не нужны: у неё есть
кнопка и полоса, а ввод она отдаёт VTE напрямую, мимо окна.

Терминалы живут в `Gtk.Stack`, по виджету на вкладку: переключение ничего не
пересобирает, меняется только видимая страница. Схема, шрифт и его размер —
общие: они хранятся в окне, применяются сразу ко всем вкладкам, и новая
открывается с тем же оформлением. Клавиатура, поиск и полоса перемотки всегда
работают с открытой вкладкой — `Window.term` возвращает именно её.
...
клавиш в наложении, режимы наложения, убранной клавиатуры и полного экрана,
размер окна.
...
terkb/
  ...
  tabs.py      вкладки: полоса кнопок и стопка терминалов
  window.py    окно: панель, вкладки, режимы, поиск, правка макросов
...
```bash
python3 tests/test_tabs.py
```

Вкладки — в offscreen-окне, как основной тест: проверяется, что при запуске
вкладка одна и полосы нет, что `＋` открывает вторую с тем же оформлением и
полоса появляется (отъедая высоту у клавиатуры), что ввод с экранной
клавиатуры уходит только в открытую вкладку и переезжает вместе с
переключением, что схема и размер шрифта общие на все вкладки, что сверх
предела вкладка не открывается, а кнопка `＋` на нём гаснет, что крестик
закрывает вкладку и возвращает полосу и высоту на место, что выход из шелла
закрывает вкладку сам, а закрытие последней закрывает окно.
...
Файлы иконки — тем же загрузчиком, каким их читает оболочка: SVG в размерах,
под которые нет готового PNG (44, 56, 96), и все растровые копии. Ловит случай,
когда картинка на вид цела и открывается в просмотрщике, но gdk-pixbuf её не
опознаёт, и док показывает чёрный квадрат.
